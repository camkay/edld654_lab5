...
```{r}
select_best(rf_tune_fit, "rmse") #mtry = 15, min_n = 40
select_best(rf_tune_fit, "rsq") #mtry = 15, min_n = 40
select_best(rf_tune_fit, "huber_loss") #mtry = 22, min_n = 20
...
*Bagged tree models* generally split at variables that are most relevant to predicting the outcome variable. For instance, most of the trees will use the strongest predictor in the top split; hence trees from different resamples generally have similar structure to each other (especially at the top of the tree). Conversely, *random forest models* aim to reduce underlying strong relations across trees by splitting at a randomly selected root variable (which is limited to a random subset of the original predictors).
