...
I calculated two OOB *rmse* values. The first *rmse* was obtained when using the workflow object where I included the tuning parameters. Since the inclusion of these tuning parameters produced a series of troublesome warnings and ultimately resulted in a much larger *rmse*, I will hereafter ignore this model. The second *rmse* was obtained when using the workflow object where I deleted the tuning parameters for `mtry` and `min_n`. In this case, the OOB *rmse* (87.07) is actually quite similar to the *rmse* from our 10-fold cross validation random forest model (*rmse* = 87.5). Given how well the data are predicted, 10-fold cross validation appears to produce models with relatively low bias (i.e., high performance). While a bias-variance trade-off would suggest that 10-fold cross validation may therefore have more variability, the variability wasn't too bad. That is, the results did not change drastically when changing samples (see `rf_res_final` above). The *rmse* actually improved when conducting the final fit. As such, 10-fold cross validation seems to do a fine job resolving any trade-off between bias-variance. This makes sense considering the model fitting process repeats over 10 "folds", and each fold contains a sample in which none of the observations are repeated.
