...
dt_tune_fit %>%
  show_best(n = 1, metric = "rmse") # rmse = 95.4

dt_tune_fit %>%
  show_best(n = 1, metric = "rsq") #rsq = .302

dt_tune_fit %>%
  show_best(n = 1, metric = "huber_loss") #huber loss = 74.6

# in each case, a cost complexity of 0.0401 and min_n of 9 is best!  
...
# in each case, a cost complexity of 0.00228 and a min_n of 36 is best!
```
...
```{r}
sqrt(rf_res_final$fit$fit$fit$prediction.error) #rmse = 86.94
```
...
The OOB *rmse* here is 86.94, which is very similar--albeit higher--than the *rmse* we obtained via 10-fold cross validation (86.7). Given the similarity in prediction/how well the data are predicted, 10-fold cross validation appears to produce models with low bias (i.e., high performance). While a bias-variance trade-off would suggest that 10-fold cross validation may therefore have more variability, it actually had low variability here (i.e., results did not change much across samples). As such, 10-fold cross validation seems to do a fine job resolving any trade-off between bias-variance.
